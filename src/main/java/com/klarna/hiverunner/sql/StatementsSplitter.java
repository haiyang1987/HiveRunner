...
/**
 * Splits hive sql statements into executable elements. Input will be split on ';'. ';' in comments (--) or quotes (' or
 * ") will be ignored. Trailing whitespaces and empty elements caused by multiple ';' will be removed.
 * <p/>
 * E.g: foo;bar baz -- comment with ; 'fox;'; love
 * <p/>
 * will be split into
 * <p/>
 * [foo, bar baz -- comment with ; 'fox;', love]
 */
public class StatementsSplitter {
  ...
  /**
   * Splits expression on ';'. ';' within quotes (" or ') or comments ( -- ) are ignored.
   */
  ...
  public static List<String> splitStatementsRemoveComments(String expression) {
    StringTokenizer tokenizer = new StringTokenizer(expression, SQL_SPECIAL_CHARS, true);
    ...
    while (tokenizer.hasMoreElements()) {
      ...
      case "-":
        ...
        if (START_OF_COMMENT_PATTERN.matcher(statement).matches()) {
          ...
          readUntilEndOfLine(tokenizer); // gets the whole comment
          statement = statement.substring(0, statement.length() - 2); // removes the prefix '--'
        }
        break;
